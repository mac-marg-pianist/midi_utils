...
def add_pedal_inf_to_notes(midi_obj):
    ...
    for note in notes:
        ...
        note.pedal_at_start = sustain_pedals[pedal_index_at_note_start].value > threshold
        note.pedal_at_end = sustain_pedals[pedal_index_at_note_end].value > threshold
        note.soft_pedal = soft_pedals[soft_pedal_index].value > soft_threshold
        ...
        note.pedal_refresh, note.pedal_refresh_time = \
            check_pedal_refresh_in_note(note, notes, sustain_pedals,sustain_pedals_positions,
        ...
        note.pedal_cut, note.pedal_cut_time =\
            check_pedal_cut(note, notes, sustain_pedals, sustain_pedals_positions)
...
def check_pedal_refresh_in_note(note, notes, pedals, pedals_positions, pd_ind1, pd_ind2, threshold=30):
    ...
    for i in range(pd_ind1, pd_ind2):
        ...
        if pedal.value < threshold:
            time_ratio = (pedal.start - note.start) / (search_time_end - note.start)
            return True, time_ratio
    return False, 0

def check_pedal_cut(note, notes, pedals, pedals_positions, threshold=30):
    note_index = notes.index(note)
    if note_index == 0:
        return False, 0
    ...
    last_note = notes[note_index-1]

    if last_note.pedal_at_end:
        pd1 = binaryIndex(pedals_positions, last_note.end)
        pd2 = binaryIndex(pedals_positions, note.start)
        for i in range(pd1, pd2):
            pedal = pedals[i]
            if pedal.value < threshold:
                time_ratio = (note.start - pedal.start) / (note.start - last_note.end)
                return True, time_ratio
...
def save_note_pedal_to_CC(midi_obj):
    ...
    for i in range(num_notes):
        ...
        pedal1 = pretty_midi.ControlChange(number=64, value=to_8(note.pedal_at_start), time=note.start-ep)
        pedal2 = pretty_midi.ControlChange(number=64, value=to_8(note.pedal_at_end), time=note.end-ep)
        soft_pedal = pretty_midi.ControlChange(number=67, value=to_8(note.soft_pedal), time=note.start-ep)
        ...
        if note.pedal_refresh:
            end_time = note.end
            if i < num_notes-1:
                end_time = notes[i+1].start
            refresh_time = note.start + (end_time - note.start) * note.pedal_refresh_time
            pedal3 = pretty_midi.ControlChange(number=64, value=0, time=refresh_time)

            instrument.control_changes.append(pedal3)

        if note.pedal_cut:
            cut_time = note.start - (note.start - notes[i-1].end) * note.pedal_cut_time
            pedal4 = pretty_midi.ControlChange(number=64, value=0, time=cut_time)
